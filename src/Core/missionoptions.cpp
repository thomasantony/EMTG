...
namespace EMTG
{
    ...
    missionoptions::missionoptions()
    {
        ...
        this->NLP_write_output_check_time = 1000000;
        this->MBH_RNG_seed = -1;
        ...
        this->print_only_non_default_options = (bool) 0;
        ...
        this->forward_integrated_ephemeris_minimum_timestep_kept = 120;
        ...
        this->checkFeasibilityTolInMBHToSkipNLP = (bool) 0;
        this->feasibilityTolInMBHToSkipNLP = 1000000;
        this->MBH_max_not_improve_with_NLP_skip = 10000;
        ...
        this->NLP_write_output_check_time_upperBound = INT_MAX;
        ...
        this->PeriapseBoundaryStateRepresentation_upperBound = (StateRepresentation) 8;
        ...
        this->forward_integrated_ephemeris_minimum_timestep_kept_lowerBound = 0.00001;
        this->forward_integrated_ephemeris_minimum_timestep_kept_upperBound = math::LARGE;
        ...
        this->feasibilityTolInMBHToSkipNLP_lowerBound = 1.00E-10;
        this->feasibilityTolInMBHToSkipNLP_upperBound = 1.00E+10;
        this->MBH_max_not_improve_with_NLP_skip_lowerBound = 1;
        this->MBH_max_not_improve_with_NLP_skip_upperBound = INT_MAX;
    }//end constructor
    ...
    void missionoptions::parse_line(const std::string& line, size_t& lineNumber)
    {
        ...
        if (linecell[0] == "NLP_write_output_check_time")
        {
            this->NLP_write_output_check_time = std::stoi(linecell[1]);
            
            //bounds check
            if (this->NLP_write_output_check_time < this->NLP_write_output_check_time_lowerBound || this->NLP_write_output_check_time > this->NLP_write_output_check_time_upperBound)
            {
                throw std::out_of_range("Input option NLP_write_output_check_time is out of bounds on line " + std::to_string(lineNumber) + ". Value is " + std::to_string(this->NLP_write_output_check_time) + ", bounds are [" + std::to_string(this->NLP_write_output_check_time_lowerBound) + ", " + std::to_string(this->NLP_write_output_check_time_upperBound) + "].");
            }
            return;
        }
        ...
        if (linecell[0] == "forward_integrated_ephemeris_minimum_timestep_kept")
        {
            this->forward_integrated_ephemeris_minimum_timestep_kept = std::stod(linecell[1]);
            
            //bounds check
            if (this->forward_integrated_ephemeris_minimum_timestep_kept < this->forward_integrated_ephemeris_minimum_timestep_kept_lowerBound || this->forward_integrated_ephemeris_minimum_timestep_kept > this->forward_integrated_ephemeris_minimum_timestep_kept_upperBound)
            {
                throw std::out_of_range("Input option forward_integrated_ephemeris_minimum_timestep_kept is out of bounds on line " + std::to_string(lineNumber) + ". Value is " + std::to_string(this->forward_integrated_ephemeris_minimum_timestep_kept) + ", bounds are [" + std::to_string(this->forward_integrated_ephemeris_minimum_timestep_kept_lowerBound) + ", " + std::to_string(this->forward_integrated_ephemeris_minimum_timestep_kept_upperBound) + "].");
            }
            return;
        }
        ...
        if (linecell[0] == "checkFeasibilityTolInMBHToSkipNLP")
        {
            this->checkFeasibilityTolInMBHToSkipNLP = (bool) std::stoi(linecell[1]);
            return;
        }
        if (linecell[0] == "feasibilityTolInMBHToSkipNLP")
        {
            this->feasibilityTolInMBHToSkipNLP = std::stod(linecell[1]);
            
            //bounds check
            if (this->feasibilityTolInMBHToSkipNLP < this->feasibilityTolInMBHToSkipNLP_lowerBound || this->feasibilityTolInMBHToSkipNLP > this->feasibilityTolInMBHToSkipNLP_upperBound)
            {
                throw std::out_of_range("Input option feasibilityTolInMBHToSkipNLP is out of bounds on line " + std::to_string(lineNumber) + ". Value is " + std::to_string(this->feasibilityTolInMBHToSkipNLP) + ", bounds are [" + std::to_string(this->feasibilityTolInMBHToSkipNLP_lowerBound) + ", " + std::to_string(this->feasibilityTolInMBHToSkipNLP_upperBound) + "].");
            }
            return;
        }
        if (linecell[0] == "MBH_max_not_improve_with_NLP_skip")
        {
            this->MBH_max_not_improve_with_NLP_skip = std::stoi(linecell[1]);
            
            //bounds check
            if (this->MBH_max_not_improve_with_NLP_skip < this->MBH_max_not_improve_with_NLP_skip_lowerBound || this->MBH_max_not_improve_with_NLP_skip > this->MBH_max_not_improve_with_NLP_skip_upperBound)
            {
                throw std::out_of_range("Input option MBH_max_not_improve_with_NLP_skip is out of bounds on line " + std::to_string(lineNumber) + ". Value is " + std::to_string(this->MBH_max_not_improve_with_NLP_skip) + ", bounds are [" + std::to_string(this->MBH_max_not_improve_with_NLP_skip_lowerBound) + ", " + std::to_string(this->MBH_max_not_improve_with_NLP_skip_upperBound) + "].");
            }
            return;
        }
    ...
    void missionoptions::write(std::string optionsFileName, const bool& writeAll)
    {
        ...
        if (this->NLP_write_output_check_time != 1000000 || writeAll)
        {
            optionsFileStream << "#Within an NLP run, how frequently do we check to see if we have found a feasible solution with a better objective function value than we have written out before, and, if true, write it to file? (seconds) ONLY WORKS IF NLP CHAPERONE IS ACTIVE." << std::endl;
            optionsFileStream << "NLP_write_output_check_time " << this->NLP_write_output_check_time << std::endl;
        }
        ...
        {
            optionsFileStream << "#power source type. Choices are 0 - fixed, 1 - solar" << std::endl;
        ...
        {
            optionsFileStream << "#state representation for periapse boundary conditions (Cartesian, SphericalRADEC, SphericalAZFPA, COE, MEE, IncomingBplane, OutgoingBplane, IncomingBplaneRpTA, OutgoingBplaneRpTA)" << std::endl;
        ...
        if (this->forward_integrated_ephemeris_minimum_timestep_kept != 120 || writeAll)
        {
            optionsFileStream << "#Consecutive lines in ephemeris file separated by less than this amount of time will be removed when the clean ephemeris file is created. (Seconds)" << std::endl;
            optionsFileStream << "forward_integrated_ephemeris_minimum_timestep_kept " << this->forward_integrated_ephemeris_minimum_timestep_kept << std::endl;
        }
        ...
        if (this->feasibilityTolInMBHToSkipNLP != 1000000 || writeAll)
        {
            optionsFileStream << "#If checkFeasibilityTolInMBHToSkipNLP is true, then we skip the NLP solve for any MBH iteration whose initial feasibility is greater than the value of this option." << std::endl;
            optionsFileStream << "feasibilityTolInMBHToSkipNLP " << this->feasibilityTolInMBHToSkipNLP << std::endl;
        }
    
        if (this->MBH_max_not_improve_with_NLP_skip != 10000 || writeAll)
        {
            optionsFileStream << "#Maximum number of hops without improving objective function before trying a random point when the guess is worse than the skip-NLP-solve-tolerance and checkFeasibilityTolInMBHToSkipNLP is true." << std::endl;
            optionsFileStream << "MBH_max_not_improve_with_NLP_skip " << this->MBH_max_not_improve_with_NLP_skip << std::endl;
        }
