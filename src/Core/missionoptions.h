...
namespace EMTG
{
    ...
    public:
        ...
        int MBH_hop_distribution;//MBH hop probabilty distribution. Choices are 0 - uniform, 1 - Cauchy, 2 - Pareto, 3 - Gaussian
        ...
        int NLP_write_output_check_time;//Within an NLP run, how frequently do we check to see if we have found a feasible solution with a better objective function value than we have written out before, and, if true, write it to file? (seconds) ONLY WORKS IF NLP CHAPERONE IS ACTIVE.
        ...
        SpacecraftPowerSupplyType power_source_type;//power source type. Choices are 0 - fixed, 1 - solar
        SpacecraftPowerSupplyCurveType solar_power_model_type;//Solar power model type. Choices are 0 - classic Sauer model, 1 - polynomial (0th order on the left)
        ...
        ThrottleLogic throttle_logic_mode;//Throttle logic mode. Choices are 0 - maximum number of thrusters, 1 - minimum number of thrusters
        ...
        SpacecraftModelInputType SpacecraftModelInput;//Spacecraft object input type. Choices are 0 - Assemble from libraries, 1 - Read .emtg_spacecraftoptions file, 2 - Assemble from missionoptions object
        ...
        StateRepresentation PeriapseBoundaryStateRepresentation;//state representation for periapse boundary conditions (Cartesian, SphericalRADEC, SphericalAZFPA, COE, MEE, IncomingBplane, OutgoingBplane, IncomingBplaneRpTA, OutgoingBplaneRpTA)
        ...
        ReferenceFrame output_file_frame;//reference frame for output file. Choices are 0 - J2000_ICRF, 1 - J2000_BCI, 2 - J2000_BCF, 3 - TrueOfDate_BCI, 4 - TrueOfDate_BCF, 5 - Principle Axes, 6 - Topocentric, 7 - Polar
        ...
        double forward_integrated_ephemeris_minimum_timestep_kept;//Consecutive lines in ephemeris file separated by less than this amount of time will be removed when the clean ephemeris file is created. (Seconds)
        ...
        bool checkFeasibilityTolInMBHToSkipNLP;//Should EMTG check the feasibility tolerance of the initial guess when running in MBH mode and skip the NLP solve for that iteration if the feasibility tolerance is greater than feasibilityTolInMBHToSkipNLP?
        double feasibilityTolInMBHToSkipNLP;//If checkFeasibilityTolInMBHToSkipNLP is true, then we skip the NLP solve for any MBH iteration whose initial feasibility is greater than the value of this option.
        int MBH_max_not_improve_with_NLP_skip;//Maximum number of hops without improving objective function before trying a random point when the guess is worse than the skip-NLP-solve-tolerance and checkFeasibilityTolInMBHToSkipNLP is true.
    ...
    private:
        ...
        int NLP_write_output_check_time_upperBound;
        ...
        double forward_integrated_ephemeris_minimum_timestep_kept_lowerBound;
        double forward_integrated_ephemeris_minimum_timestep_kept_upperBound;
        ...
        double feasibilityTolInMBHToSkipNLP_lowerBound;
        double feasibilityTolInMBHToSkipNLP_upperBound;
        int MBH_max_not_improve_with_NLP_skip_lowerBound;
        int MBH_max_not_improve_with_NLP_skip_upperBound;
