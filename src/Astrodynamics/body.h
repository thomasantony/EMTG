...
namespace EMTG
{
    namespace Astrodynamics
    {
        ...
        public:
            ...
            inline void setHarmonicFieldDegreeOrder(const size_t& degree, const size_t& order) { this->harmonic_gravity_field->setDegreeAndOrder(degree, order); };

            //comparisons
            virtual bool operator== (const body& OtherBody) const;
            virtual bool operator!= (const body& OtherBody) const;

            double getEphemerisWindowOpen() const {
#ifdef SPLINE_EPHEM
                if (this->body_ephemeris_source == 2)
                    return this->MySplineUniverse->getEphemerisWindowOpen(this->spice_ID, this->central_body_spice_ID);
#endif
                return this->ephemeris_window_open;
            }
            double getEphemerisWindowClose() const {
#ifdef SPLINE_EPHEM
                if (this->body_ephemeris_source == 2)
                    return this->MySplineUniverse->getEphemerisWindowClose(this->spice_ID, this->central_body_spice_ID);
#endif
                return this->ephemeris_window_close;
            }
            ...
            std::shared_ptr<HarmonicGravityField> harmonic_gravity_field;
        ...
        private:
            ...
            bool mass_coeffs_are_normalized;
