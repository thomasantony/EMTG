...
class MissionOptions(object):
    ...
    def __init__(self, optionsFileName = None):
        ...
        """mission name"""
        self.objective_type = 2
        """objective type, see ObjectiveFunctionType in EMTG_enums.h"""
        ...
        self.launch_window_open_date = 53782
        """launch window open date (MJD)"""
        self.total_flight_time_bounds = [0.0, 302]
        """flight time bounds (days)"""
        self.DLA_bounds = [-90.0, 90.0]
        """DLA in degrees"""
        ...
        self.quiet_NLP = 1
        """Quiet NLP solver?"""
        self.ACE_feasible_point_finder = 0
        ...
        self.MBH_max_not_improve = 10000
        """quantity Max_not_improve for MBH"""
        self.MBH_max_trials = 1000000
        """maximum number of trials for MBH"""
        self.MBH_max_run_time = 60
        """maximum run time for MBH, in seconds"""
        self.MBH_max_step_size = 1
        """Maximum step size (uniform) or scale factor (other distributions)"""
        self.MBH_hop_distribution = 2
        """MBH hop probabilty distribution. Choices are 0 - uniform, 1 - Cauchy, 2 - Pareto, 3 - Gaussian"""
        ...
        self.NLP_objective_goal = 0
        """Objective value at which NLP stops"""
        self.NLP_write_output_check_time = 1000000
        ...
        """Quiet MBH?"""
        self.SPICE_leap_seconds_kernel = "naif0012.tls"
        ...
        self.SplineEphem_truncate_ephemeris_at_maximum_mission_epoch = 0
        """Truncate SplineEphem at maximum mission epoch?"""
        self.earliestPossibleEpoch = 51554.5
        """Beginning of the SplineEphem data set"""
        self.latestPossibleEpoch = 88058
        """End of the SplineEphem data set"""
        self.propagatorType = 0
        """Propagator type. Choices are 0 - Keplerian propagator, 1 - Integrated propagator"""
        ...
        """duty cycle type. Choices are 0 - averaged, 1 - realistic"""
        self.thrust_scale_factor = 1
        """scale factor to apply to thrust ONLY, not mass flow rate, can use to approximate cosine loss"""
        self.power_at_1_AU = 5
        """Power at 1 AU (kW)"""
        self.power_source_type = 1
        """power source type. Choices are 0 - fixed, 1 - solar"""
        self.solar_power_model_type = 0
        """Solar power model type. Choices are 0 - classic Sauer model, 1 - polynomial (0th order on the left)"""
        self.solar_power_gamma = [1.32077,-0.10848,-0.11665,0.10843,-0.01279,0.0,0.0]
        ...
        self.power_decay_rate = 0
        """power system decay rate (fraction per year)"""
        self.power_system_decay_reference_epoch = 51544.5
        """reference date when the power system begins to decay"""
        self.throttle_sharpness = 100
        """Throttle sharpness (higher means more precise, lower means smoother)"""
        self.throttle_logic_mode = 1
        """Throttle logic mode. Choices are 0 - maximum number of thrusters, 1 - minimum number of thrusters"""
        self.spacecraft_power_coefficients = [0,0,0]
        """spacecraft power coefficients A, B, and C represent the power requirements of the spacecraft at a distance r from the sun, #i.e. heaters, communications, etc"""
        self.engine_input_thrust_coefficients = [26.337459,-51.694393,90.486509,-36.720293,5.145602,0.0,0.0]
        """thrust polynomial coefficients"""
        self.engine_input_mass_flow_rate_coefficients = [2.5060,-5.3568,6.2539,-2.5372,0.36985,0.0,0.0]
        """mass flow rate polynomial coefficients"""
        self.engine_input_power_bounds = [0.525, 2.6]
        ...
        """ThrottleTableFile"""
        self.LaunchVehicleLibraryFile = "default.emtg_launchvehicleopt"
        """LaunchVehicleLibraryFile"""
        self.PowerSystemsLibraryFile = "default.emtg_powersystemsopt"
        """PowerSystemsLibraryFile"""
        self.PropulsionSystemsLibraryFile = "default.emtg_propulsionsystemopt"
        ...
        self.ParallelShootingConstraintStateRepresentation = 0
        """state representation for parallel shooting constraints (Cartesian or same as encoded states)"""
        self.print_only_non_default_options = 0
        """Write only options that are *not* default into the .emtgopt file?"""
        self.output_file_frame = 1
        """reference frame for output file. Choices are 0 - J2000_ICRF, 1 - J2000_BCI, 2 - J2000_BCF, 3 - TrueOfDate_BCI, 4 - TrueOfDate_BCF, 5 - Principle Axes, 6 - Topocentric, 7 - Polar"""
        self.override_default_output_file_name = 0
        """Override default output file name?"""
        self.forced_output_file_name = "forced_output_file_name"
        """User-defined output file name"""
        self.output_dormant_journeys = 0
        """Output ephemeris for time spent waiting between journeys?"""
        self.post_mission_wait_time = 0
        ...
        """Mission subfolder"""
        self.short_output_file_names = 1
        """Shorten output file names?"""
        self.generate_forward_integrated_ephemeris = 0
        """Generate forward-integrated ephemeris?"""
        self.forward_integrated_ephemeris_minimum_timestep_kept = 120
        ...
        self.forward_integrated_ephemeris_central_body_SPICE_ID = 10
        ...
        self.generate_acceleration_model_instrumentation_file = 0
        ...
        while True:
            ...
            if line.strip('\r') != "":
                if line[0] != "#":
                    ...
                    elif linecell[0] == "NLP_write_output_check_time":
                        self.NLP_write_output_check_time = int(linecell[1])
                    ...
                    elif linecell[0] == "forward_integrated_ephemeris_minimum_timestep_kept":
                        self.forward_integrated_ephemeris_minimum_timestep_kept = float(linecell[1])
                    ...
                    elif linecell[0] == "checkFeasibilityTolInMBHToSkipNLP":
                        self.checkFeasibilityTolInMBHToSkipNLP = int(linecell[1])
                  
                    elif linecell[0] == "feasibilityTolInMBHToSkipNLP":
                        self.feasibilityTolInMBHToSkipNLP = float(linecell[1])
                  
                    elif linecell[0] == "MBH_max_not_improve_with_NLP_skip":
                        self.MBH_max_not_improve_with_NLP_skip = int(linecell[1])
    ...
    def write_options_file(self, optionsFileName, writeAll = False):
        ...
        with open(optionsFileName, "w+") as optionsFile:
            ...
            if (self.NLP_write_output_check_time != 1000000 or writeAll):
                optionsFile.write("#Within an NLP run, how frequently do we check to see if we have found a feasible solution with a better objective function value than we have written out before, and, if true, write it to file? (seconds) ONLY WORKS IF NLP CHAPERONE IS ACTIVE.\n")
                optionsFile.write("NLP_write_output_check_time " + str(self.NLP_write_output_check_time) + "\n")
            ...
            if (self.power_source_type != 1 or writeAll):
                optionsFile.write("#power source type. Choices are 0 - fixed, 1 - solar\n")
            ...
            if (self.PeriapseBoundaryStateRepresentation != 1 or writeAll):
                optionsFile.write("#state representation for periapse boundary conditions (Cartesian, SphericalRADEC, SphericalAZFPA, COE, MEE, IncomingBplane, OutgoingBplane, IncomingBplaneRpTA, OutgoingBplaneRpTA)\n")
            ...
            if (self.forward_integrated_ephemeris_minimum_timestep_kept != 120 or writeAll):
                optionsFile.write("#Consecutive lines in ephemeris file separated by less than this amount of time will be removed when the clean ephemeris file is created. (Seconds)\n")
                optionsFile.write("forward_integrated_ephemeris_minimum_timestep_kept " + str(self.forward_integrated_ephemeris_minimum_timestep_kept) + "\n")
            ...
            if (self.feasibilityTolInMBHToSkipNLP != 1000000 or writeAll):
                optionsFile.write("#If checkFeasibilityTolInMBHToSkipNLP is true, then we skip the NLP solve for any MBH iteration whose initial feasibility is greater than the value of this option.\n")
                optionsFile.write("feasibilityTolInMBHToSkipNLP " + str(self.feasibilityTolInMBHToSkipNLP) + "\n")
    
            if (self.MBH_max_not_improve_with_NLP_skip != 10000 or writeAll):
                optionsFile.write("#Maximum number of hops without improving objective function before trying a random point when the guess is worse than the skip-NLP-solve-tolerance and checkFeasibilityTolInMBHToSkipNLP is true.\n")
                optionsFile.write("MBH_max_not_improve_with_NLP_skip " + str(self.MBH_max_not_improve_with_NLP_skip) + "\n")
